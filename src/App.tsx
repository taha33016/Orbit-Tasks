...
import { Activity, CalendarDays, Check, Clock3, FolderKanban, GripVertical, LayoutDashboard, Plus, Trash2, UserCircle2, Users, X } from 'lucide-react'
...
export default function App() {
  ...
  const [people, setPeople] = useState<Person[]>(() => {
    try { return JSON.parse(localStorage.getItem('orbit-people') || 'null') || initialPeople } catch { return initialPeople }
  })
  const [currentUserId, setCurrentUserId] = useState(() => localStorage.getItem('orbit-current-user') || 'p1')
  const [history, setHistory] = useState<History[]>([])
  const [dragId, setDragId] = useState<string | null>(null)
  const [modal, setModal] = useState<'task' | 'project' | 'group' | 'account' | null>(null)
  ...
  const moveTask = (taskId: string, status: Status) => {
    ...
    setHistory(prev => [{ id: uid('h'), personId: currentUser.id, action: `${statusMeta[status].title} شد`, taskId, projectId: task.projectId, at: new Date().toISOString() }, ...prev])
    setDragId(null)
  }
  ...
  const addProject = (name: string, start: string, end: string, groupId: string) => { if (name.trim()) { const id = uid('pr'); setProjects(p => [...p, { id, name: name.trim(), start, end, groupId, ownerId: currentUser.id }]); setSelectedProject(id) }; setModal(null) }
  const registerPerson = (name: string, specialty: string, age: number, role: Role) => {
    const existing = people.find(p => p.id === currentUserId)
    if (existing) {
      const updated = people.map(p => p.id === currentUserId ? { ...p, name, specialty, age, role } : p)
      savePeople(updated)
    } else {
      const id = uid('p')
      savePeople([...people, { id, name, specialty, age, role }])
      setCurrentUserId(id)
      localStorage.setItem('orbit-current-user', id)
    }
    setModal(null)
  }
...
function AccountForm({person,onSubmit}:{person:Person;onSubmit:(name:string,specialty:string,age:number,role:Role)=>void}) { const [name,setName]=useState(person?.name||'');const [specialty,setSpecialty]=useState(person?.specialty||'');const [age,setAge]=useState(String(person?.age||18));const [role,setRole]=useState<Role>(person?.role||'user');return <form onSubmit={e=>{e.preventDefault();if(name.trim()&&specialty.trim()&&Number(age)>0) onSubmit(name.trim(),specialty.trim(),Number(age),role)}} className="space-y-3"><div className="rounded-xl bg-violet-500/10 border border-violet-500/15 p-3 text-xs leading-6 text-slate-400">این اطلاعات در پروفایل کاری شما ثبت می‌شود و سطح دسترسی شما برای اعضای تیم قابل مشاهده است.</div><label className="block text-xs text-slate-500">نام و نام خانوادگی<input autoFocus className="field mt-1" placeholder="مثلاً علی احمدی" value={name} onChange={e=>setName(e.target.value)}/></label><label className="block text-xs text-slate-500">تخصص<input className="field mt-1" placeholder="مثلاً برنامه‌نویس، طراح UI/UX" value={specialty} onChange={e=>setSpecialty(e.target.value)}/></label><label className="block text-xs text-slate-500">سن<input type="number" min="1" max="120" className="field mt-1" value={age} onChange={e=>setAge(e.target.value)}/></label><div><p className="mb-2 text-xs text-slate-500">سطح مدیریت / دسترسی</p><div className="grid grid-cols-2 gap-2">{([['user','کاربر','دسترسی معمولی'],['manager','مدیر','مدیریت تیم و پروژه']] as const).map(([id,label,desc])=><button type="button" key={id} onClick={()=>setRole(id)} className={`rounded-xl border p-3 text-right ${role===id?'border-violet-500 bg-violet-500/10':'border-white/10 bg-white/[.03]'}`}><b className="block text-sm text-white">{label}</b><span className="text-[10px] text-slate-500">{desc}</span></button>)}</div></div><button className="btn-primary w-full justify-center">ثبت اطلاعات</button></form> }
